...
output_image = "DistanceContour_FlowMag_Rotation2.png"  # Output image file for the plot
...
plot_title = "Flow Magnitude as colourmap and Contour Rotation"  # Title for the plot
...
def main():
    ...
    contour_levels = np.linspace(-contour_constant, -0.01, num_contours) # change this so zero isnt included

    # Generate contours based on flow speed instead of signed distance

    CS = ax.contour(lon, lat, signed_dist, levels=contour_levels, linewidths=0.8, colors='none')
    ...
    for level_index, level_value in enumerate(CS.levels):
        ...
        for seg in segs_for_level:
            ...
            for i in range(0, len(seg), chunk_size):
                ...
                angle = (local_flow / max_flow) * (np.pi / 2)
