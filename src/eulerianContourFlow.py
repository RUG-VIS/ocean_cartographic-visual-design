...
output_image = "Stippling.png"  # Output image file for the plot
shoreline_tol = 35  # Tolerance to treat elevation≈0 as shoreline
contour_constant = 0.5  # Contour interval for the signed distance field
...
def main():
    ...
    plt.savefig("signed_distance_field.png", dpi=300)
    #'''
    ...
    #contour_levels = np.linspace(-contour_constant, -0.02, num_contours) # change this so zero isnt included
    #CS= ax.contour(lon, lat, signed_dist, levels=contour_levels, colors="none", linewidths=0.8)
    ...
    def flow_to_gap_len(flow, max_flow):
 
        min_gap = 1.0
        max_gap = 10.0
        scaled = (flow / max_flow)
        # Clamp scaled between 0 and 1 if needed
        scaled = max(0.0, min(1.0, scaled))
        return min_gap + scaled * (max_gap - min_gap)
    ...
    for level_index, level_value in enumerate(CS.levels):
        ...
        for seg in segs_for_level:
            ...
            for xy_sub in sub_segs:
                ...
                if stippling_mode:
                    # Small dash length = the 'dot'
                    dash_len = 0.5
                    # Gap length depends on local_flow
                    gap_len  = flow_to_gap_len(local_flow, max_flow)
                    dash_pattern = (dash_len, gap_len)
                else:
                    # Original mode: dash length depends on flow
                    dash_len    = flow_to_dash_len(local_flow, max_flow)
                    dash_pattern = (dash_len, dash_len)
